...
def runtests(options=None):
    ...
    failures = test_runner.run_tests(labels or ['django_premailer'])
    sys.exit(failures)


if __name__ == '__main__':
    parser = ArgumentParser(description="Run the django-premailer test suite.")
    parser.add_argument('labels', nargs='*', metavar='label',
        help='Optional path(s) to test modules; e.g. "django_premailer.tests.templatetags".')
    parser.add_argument(
